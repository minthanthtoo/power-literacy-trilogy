...
```mermaid
graph TD
    O(Operator / Whisperer) -.-> N1(Node 1: Board Member)
    N1 -- Negative Signal --> N2(Node 2: LP)
    N1 -- Negative Signal --> N3(Node 3: Key Talent)
    N2 -- Score Downgrade --> T[Target]
    N3 -- Score Downgrade --> T
    N1 -- Score Downgrade --> T
```
...
```mermaid
graph TD
    A[Operator Sets Agenda] --> B[Present False Choice A: Total Liquidation]
    A --> C[Present False Choice B: Hostile Takeover]
    B --> D{Target Analyzes Friction}
    C --> D
    D --> E[Operator Presents Option C: The 'Compromise']
    E --> F[Target Accepts True Objective]
```
